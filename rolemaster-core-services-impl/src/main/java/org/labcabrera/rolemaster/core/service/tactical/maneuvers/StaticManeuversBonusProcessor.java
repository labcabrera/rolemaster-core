...
@Component
public class StaticManeuversBonusProcessor {

	@Autowired
	private TacticalSkillService skillService;

	public Mono<Map<String, Integer>> getBonusMap(TacticalActionStaticManeuver action, TacticalCharacter tc, Map<String, Integer> map) {
		return Mono.zip(Mono.just(action), Mono.just(tc), Mono.just(map))
			.flatMap(this::processSkill)
			.map(Tuple3::getT3);
	}

	private Mono<Tuple3<TacticalActionStaticManeuver, TacticalCharacter, Map<String, Integer>>> processSkill(
		Tuple3<TacticalActionStaticManeuver, TacticalCharacter, Map<String, Integer>> tuple) {
